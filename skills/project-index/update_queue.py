"""Standalone copy for the project-index Hermes skill.

Deliberately self-contained (no import from ledger.py/logging_helper.py):
this file ships inside a Hermes *skill* directory, which doesn't share a
Python path with scripts/skills/ (or with the session-ledger skill that
owns ledger.py/logging_helper.py), and pulling in ledger.py would drag
python-ulid into a skill that only ever needed one small file-locking
helper. The canonical, tested copy of this module lives at
scripts/skills/update_queue.py -- keep both in sync by hand when either
changes (same convention already used for every other skill file bundled
under onboarding/hermes-template-config/, e.g. the update-announcer
plugin's own copy).
"""
import contextlib
import datetime
